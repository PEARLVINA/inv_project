...
class Manage
{
	...
	public function storeCustomerOrderInvoice($orderdate,$cust_name,$ar_tqty,$ar_qty){
		$pre_stmt = $this->con->prepare("INSERT INTO 
			`invoice`(`customer_name`, `order_date`) VALUES (?,?,?)");
		$pre_stmt->bind_param("ss",$cust_name,$orderdate);
		$pre_stmt->execute() or die($this->con->error);
